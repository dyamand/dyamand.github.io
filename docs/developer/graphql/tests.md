...
!!! success "_e.6.0.E.3_ When a client tries to set up an SSE connection after an SSE connection for that subscription ID has already been created, a 410 should be returned"
!!! success "_e.6.0.E.4_ When a client tries to set up an SSE connection but includes a header different from text/event-stream, a 406 should be returned"
...
!!! success "_e.7.0_ When a client executes a subscription with a valid subscription ID and an event arrives, a message should be sent with topic _subscriptionId_/_operationName_ and the message should be equal to the output generated by the GraphQL subscription"
!!! failure "_e.7.0.E.1_ When a client executes a subscription without a subscription ID, the subscription should fail with a 400 status code"
!!! failure "_e.7.0.E.2_ When a client executes a subscription with a subscription ID and the subscription ID is not registered, the subscription should fail with a 400 status code"
!!! failure "_e.7.1_ When a client executes a subscription with a valid subscription ID, cancels it, and an event arrives, there should be no message on topic _subscriptionId_/_operationName_"
!!! failure "_e.7.2_ When a client executes a subscription with a valid subsciption ID and an event is received on the *graphql.subscriptionRemoved* topic, all created subscriptions should be cancelled and as such, no new data on the publisher should trigger an event on the pubsub"
