...
!!! failure "_e.6.2_ When a client sets up an SSE connection and a random message is sent on Redis on topic <subscriptionId>/<eventType>, an event should be sent on the SSE connection with event == eventType and data equal to the content of the message sent on Redis"
!!! failure "_e.6.3_ When a client sets up an SSE connection and a random message is sent on Redis on topic <otherSubscriptionId>/<eventType>, no event should be received over the SSE connection"
...
!!! failure "_e.7.2_ When a client executes a subscription with a valid subscription ID and an event arrives, a message should be sent with topic <subscriptionId>/<operationName> and the message should be equal to the output generated by the GraphQL subscription"
